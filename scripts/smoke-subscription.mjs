...
const {task}=await request('/api/tasks',{prompt:artifactMode?'This is an integration smoke test. Create a real small SVG file smoke.svg in the working directory containing a green circle and label INNO. Return a structured JSON with summary INNO_SMOKE_OK, checkpoint verified, and artifacts [{name: smoke.svg, mime: image/svg+xml, path: smoke.svg}]. No subagents needed.':'This is a simple integration smoke test. Reply with exactly INNO_SMOKE_OK as summary. No tools, files, or subagents are needed.',type:'general'});
...
if(artifactMode){const a=result.artifacts.find(a=>a.name==='smoke.svg');assert.ok(a,'generated SVG artifact required');assert.match(a.encoding==='base64'?Buffer.from(a.content,'base64').toString():a.content,/<svg/);}
